...
int main(int argc,  char *argv[])
{
	...
	for (int i = 0; i < inputImageSpec.nchannels; i++)
	{
		...
		std::string imagePlaneName("RGBA");
		...
		imageChannelComponentNames.insert(std::make_pair(channelName, componentName));
	...
	{
		const auto &outputFilePath = outputFilePathNoExt + "_" + imagePlaneName + ".exr";
		...
		OIIO::ImageSpec outputImageSpec(inputImageSpec.width,
			inputImageSpec.height,
			channelCount,
			OIIO::TypeDesc::FLOAT);
		static const char *CHANNEL_NAME[4] = { "R", "G", "B", "A" };
		for (int i = 0; i < channelCount;i++)
		{
			outputImageSpec.channelformats.push_back(OIIO::TypeDesc::FLOAT);
			outputImageSpec.channelnames.push_back(CHANNEL_NAME[i]);
		}
		...
		std::cout << imagePlaneName << " " << channelCount << " " << outputImageSpec.scanline_bytes() << std::endl;
	...
	for (int y = 0;y < inputImageSpec.height;y++)
	{
		...
		for (const auto &pair : imagePlaneChannelNames)
		{
			...
			memcpy(scanline.data() + componentIndex * inputImageSpec.width,
				data.data() + channelIndex * inputImageSpec.width,
				inputImageSpec.width * sizeof(float));
		}
